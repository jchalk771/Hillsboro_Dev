function createPendingReqInspection(iGroup, iType) // optional Cap ID
{
	var itemCap = capId;
	if (arguments.length == 3)
		itemCap = arguments[2]; // use cap ID specified in args

	if (doesPendingInspectionExist(iType)) {
		logDebug("Pending inspection of type " + iType + " exists, not readding");
		return;
	}
	...
	activityModel.setRequiredInspection("Y");
